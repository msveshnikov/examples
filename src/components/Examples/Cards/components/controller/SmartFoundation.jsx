...
class SmartFoundation extends React.Component {
    static propTypes = {
        cards: T.array,
    ...
    moveCards = (card) => {
        this.props.moveCards([card], { from: card.where, to: ["FOUNDATION", this.props.suit] });
    };

    render() {
        const { connectDropTarget, isOver, suit, cards, canDrop } = this.props;
        const card = cards.length ? (
            <DraggableCard {...cards[cards.length - 1]} upturned where={["FOUNDATION", suit]} />
        ) : null;
...
export default DropTarget("DraggableCard", foundationTarget, collect)(SmartFoundation);
