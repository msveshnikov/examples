...
export const OrderedDeck = flatten(Object.keys(Suits).map((suit) => Ranks.map((rank) => ({ rank, suit }))));
...
function movingMultipleCardsFromPileToPile(where, cards) {
    return first(where.from) === Places.PILE && first(where.to) === Places.PILE && !first(cards).isLast;
}
...
function moveCards(state, action) {
    ...
    return state.updateIn(where.to, (value) => target).updateIn(where.from, (value) => source);
}
...
export default function game(state = getInitialState(shuffle(OrderedDeck)), action) {
    switch (action.type) {
